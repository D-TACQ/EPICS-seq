...
typedef seqBool SEQ_TRANS_FUNC(SS_ID ssId, int *nextState);
...
typedef struct seqWait seqWait;
...
struct seqState
{
	...
	SEQ_TRANS_FUNC	*transFunc;	/* transition routine for this state */
...
epicsShareFunc size_t seq_pvOffset(SS_ID ssId, CH_ID ch);

#define seq_pvValue(type,ch) (*(type)(seq_pvOffset(seqg_env,ch)+(size_t)seqg_var))
