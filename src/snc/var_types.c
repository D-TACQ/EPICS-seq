...
int traverse_channel_type(Type *t, on_channel_type *iter, void *arg)
{
    ...
    if (t->tag == T_PV || t->tag == T_EVFLAG) {
        if (iter(t, arg)) return TRUE;
    } else if (t->tag == T_STRUCT) {
        foreach (member, t->val.structure.member_decls) {
            if (member->tag == D_DECL)
                if (traverse_channel_type(member->extra.e_decl->type, iter, arg))
                    return TRUE;
        }
    } else if (ct) {
        if (traverse_channel_type(ct, iter, arg)) return TRUE;
    }
    return FALSE;
}
