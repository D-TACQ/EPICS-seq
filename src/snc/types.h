...
#define DEFAULT_OPTIONS {0,1,0,0,0,0,1,1,0}
...
struct syntax_node			/* generic syntax node */
{
	...
	union				/* extra data, depends on tag */
	{
		...
		Wait	*e_wait;	/* event mask for wait stmt */
	}	extra;
...
#define has_sub_scope_mask	( bit(D_ENTEX)	| bit(D_FUNCDEF)| bit(D_PROG)	\
				| bit(D_SS)	| bit(D_STATE)	| bit(D_WHEN)	\
				| bit(S_CMPND)	| bit(S_DO)	| bit(S_FOR)	\
				| bit(S_IF)	| bit(S_STMT)	| bit(S_WAIT)	\
				| bit(S_WHILE) )

/* Nodes that are actually expressions i.e. no definitions or statements.
   These are the ones that start with E_. */
#define	expr_mask		( bit(E_ASSOP)	| bit(E_BINOP)	| bit(E_CAST)	\
				| bit(E_CONST)	| bit(E_FUNC)	| bit(E_INIT)	\
...
#define do_cond		children[0]
#define do_stmt		children[1]
